...
from data_utils import HARHandler
import numpy as np
import keras
from keras import backend as K

def train_har_model(epochs, batch_size, output_dir, code_size, lr=1e-4, terms=4, predict_terms=4, image_size=64, color=True):

    # Initialize HAR data handler
    har_handler = HARHandler(video_dir='/home/paperspace/tinyfaces-cpc/video_dataset/Human Activity Recognition - Video Dataset')

    def data_generator(har_handler, batch_size, positive_samples):
        while True:
            x, y = har_handler.get_batch(batch_size, positive_samples)
            x_input = x[:, :terms]
            y_input = x[:, terms:]
            y_one_hot = np.eye(2)[y]  # Assuming binary classification: 0 (negative) or 1 (positive)
            yield [x_input, y_input], y_one_hot

    # Prepare data generators
    train_data = data_generator(har_handler, batch_size, positive_samples=True)
    validation_data = data_generator(har_handler, batch_size, positive_samples=False)

    # Prepare the model
    model = network_cpc(image_shape=(image_size, image_size, 3), terms=terms, predict_terms=predict_terms, code_size=code_size, learning_rate=lr)
    ...
    model.fit(
        train_data,
        steps_per_epoch=100,  # Assuming a fixed number of steps per epoch
        validation_data=validation_data,
        validation_steps=50,  # Assuming a fixed number of validation steps
        epochs=epochs,
        verbose=1,
        callbacks=callbacks
    )
    ...
    # Save the model
    model.save(os.path.join(output_dir, 'cpc_har.h5'))

    # Save the encoder alone
    encoder = model.layers[1].layer
    encoder.save(os.path.join(output_dir, 'encoder_har.h5'))
